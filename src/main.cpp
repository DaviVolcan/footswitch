...
#define RELAY_PIN 13          // Relay module is connected to digital pin 2
#define CURRENT_PIN 12        // Relay module is connected to digital pin 2

#define RELAY_LOGIC HIGH           // Set to LOW if your relay is activated by a ground signal. Set to HIGH if your relay is activated by a 5V signal. If your relay is active at bootup, and turns off when CURRENT_THRESHOLD is met, then try changing this setting to the opposite.
#define CURRENT_THRESHOLD 570     // The analog value above which the relay shall be triggered
#define CURRENT_SAMPLE_PERIOD 1 // The number of milliseconds to sample the current reading
#define RELAY_STARTUP_DELAY 1  // The number of milliseconds to delay before activating relay after tool has started
